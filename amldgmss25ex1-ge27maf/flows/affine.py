...
class Affine(Flow):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # log determinant of the Jacobian: log(det(J))
        # for y_i = exp(a_i) * x_i + b_i, the Jacobian is a diagonal matrix
        # with diagonal elements J_ii = d(y_i)/d(x_i) = exp(a_i).
        # the determinant of a diagonal matrix is the product of its diagonal elements.
        # log(det(J)) = log(product(exp(a_i))) = sum(log(exp(a_i))) = sum(a_i).
        # sum self.log_scale across the dimension 'dim' and then expand it to match the batch size.
        log_det_jac = torch.sum(self.log_scale).expand(B)
    ...
    def inverse(self, y: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # log determinant of the inverse Jacobian: log(det(J_inv))
        # for x_i = (y_i - b_i) / exp(a_i), the inverse Jacobian is a diagonal matrix
        # with diagonal elements J_inv_ii = d(x_i)/d(y_i) = 1 / exp(a_i) = exp(-a_i).
        # log(det(J_inv)) = log(product(exp(-a_i))) = sum(log(exp(-a_i))) = sum(-a_i) = -sum(a_i).
        # take the negative sum of self.log_scale and expand it to match the batch size.
        inv_log_det_jac = -torch.sum(self.log_scale).expand(B)
